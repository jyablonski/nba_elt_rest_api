...
def test_game_types(game_types_fixture):
    ...
    assert list(game_types_fixture[0].keys()) == [
        "game_type",
        "type",
        "n",
        "explanation",
...
def test_injuries(injuries_fixture):
    assert list(injuries_fixture[0].keys()) == [
        "player",
        "team_acronym",
        "team",
        "date",
...
def test_injuries_single(injuries_team_fixture):
    assert list(injuries_team_fixture.keys()) == [
        "player",
        "team_acronym",
        "team",
        "date",
...
def test_team_ratings(team_ratings_fixture):
    assert list(team_ratings_fixture[0].keys()) == [
        "team",
        "team_acronym",
        "w",
        "l",
...
def test_team_ratings_single(team_ratings_team_fixture):
    assert list(team_ratings_team_fixture.keys()) == [
        "team",
        "team_acronym",
        "w",
        "l",
...
def test_scorers(scorers_fixture):
    assert list(scorers_fixture[0].keys()) == [
        "player",
        "team",
        "full_team",
        "season_avg_ppg",
...
def test_reddit_comments(reddit_comments_fixture):
    assert list(reddit_comments_fixture[0].keys()) == [
        "scrape_date",
        "author",
        "comment",
        "flair",
...
def test_twitter_comments(twitter_comments_fixture):
    assert list(twitter_comments_fixture[0].keys()) == [
        "scrape_ts",
        "username",
        "tweet",
        "url",
